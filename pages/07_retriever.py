...
retriever_weight =col1.slider(
    "Retriever 가중치(높음: 의미검색, 낮음:키워드검색)", 
    min_value =0.0, 
    max_value=1.0, 
    value =0.5, 
    step =0.05
...
if confirm_btn:
    if uploaded_file:
        ...
        st.session_state ["my_retriever"]= rag_setup2(
            file_path,chunk_size=chunk_size,chunk_overlap=chunk_overlap,k=search_k, weight=retriever_weight
         )
        st.write("설정이 완료되었습니다.")
        
        st.chat_input(" 검색할 내용을_입력해 주세요")
